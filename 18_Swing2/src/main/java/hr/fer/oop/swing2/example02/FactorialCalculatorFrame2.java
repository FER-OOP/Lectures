package hr.fer.oop.swing2.example02;
...
public class FactorialCalculatorFrame2 extends JFrame {
    ...
    public FactorialCalculatorFrame2() {
        ...
        bCalculate.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent actionEvent) {
                try {
                    ...
                    //execute the task on a newly created working thread 
                    Thread thread = new Thread() {
                        @Override
                        public void run() {
                            SwingWorker fTask = new CalculateFactorialTask(number);
                            fTask.execute();

                            try {
                                System.out.println("Rezultat izvođenja koji možemo iskoristiti za novi posao: " + fTask.get());
                            } catch (InterruptedException | ExecutionException ex) {
                                //cannot be interrupted in this example
                            }

                            System.out.println("B. Izvodim li se unutar dretve Event Dispatch? " + SwingUtilities.isEventDispatchThread());
                        }
                    };
                    thread.start();

                    System.out.println("A. Izvodim li se unutar dretve Event Dispatch? " + SwingUtilities.isEventDispatchThread());
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(new Runnable() {
            @Override
            public void run() {
                FactorialCalculatorFrame2 frame = new FactorialCalculatorFrame2();
