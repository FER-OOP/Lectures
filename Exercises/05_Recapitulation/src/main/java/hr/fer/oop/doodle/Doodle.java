 ...
 * Task 2
 *
 * An arbitrary number need to fill in a Doodle in an attempt to meet during
 * the current day. The Doodle needs to ask them to fill in their names and the
 ...
 * Zadatak 2
 *
 * Proizvoljan broj osoba trebaj popuniti Doodle u pokušaju da se sastanu u toku dana.
 * Doodle ih treba pitati da navedu imena, željeno vrijeme sastanka ili ključnu riječ
 * "END" te treba ispisati njihove odgovore u tabličnom obliku, npr. :
 *
 * |  ----    |Time      |
 * |Alice     |12        |
 * |Bob       |13        |
...
public class Doodle {
	...
	public String toTable() {
		...
		sb.append(String.format("|%-10s|%-10s|%n", "  ----  ", "Time")); // 2 spaces, 4 dashes, 2 spaces

		int len = names.size();
		for (int i = 0; i < len; i++) {
			sb.append(String.format("|%-10s|%-10d|%n", names.get(i), times.get(i)));
